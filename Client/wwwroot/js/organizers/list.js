const getOrganizers = async () => {
    try {
        const result = await DataSource.getOrganizers();
        renderResult(result);
    } catch (message) {
    ...
    $('#organizer_datatable').dataTable({
        ...
        columns: [
            ...
            { data: 'link' },
            {
                ...
                render: function (data, type, row) {
                    return `
                        Facebook: ${row['facebook'] ? row['facebook'] : '-'}<br>
                        Instagram: ${row['instagram'] ? row['instagram'] : '-'}<br>
                    `;
                }
            },
            {
...
const fallbackResult = (message) => {
    const organizerListElement = document.querySelector('#organizer-list');
